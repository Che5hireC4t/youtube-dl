    ...
    if opts.tor:
        ...
        if len(available_tor_proxies) == 0:
            if opts.allow_unsafe_ports is not True:
                sys.stdout.write('\033[91mNo secure socks5 port have been found for the tor gateway. Try to reconfigure your torrc file, or use --allow-unsafe-ports option\n\033[00m')
                sys.stdout.write('\033[91mThis program is now shutting down for security reasons.\n\033[00m')
                exit(0)
            else:
                available_tor_proxies = [p['http'] for p in tor.get_socks_proxy_dict(use_recommended_flags=False)]
                if len(available_tor_proxies) == 0:
                    sys.stdout.write('\033[91mNo socks5 port has been found to interact with tor. Check the torrc file.\033[00m')
                    exit(0)
